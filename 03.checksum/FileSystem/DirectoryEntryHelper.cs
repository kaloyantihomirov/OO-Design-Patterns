...
{
    private static DirectoryEntry BuildDirectoryTree(string rootPath)
    {
    ...
    public static DirectoryEntry BuildRootDirectoryEntryInMem(
        string path, 
        bool followSymLinks)
    {
        if (followSymLinks)
        {
            ...
            builder.StartDirectory(path);
            return builder.GetResult();
        }

        return BuildDirectoryTree(path);
    }
}